 ...
 */
//#ifdef __CYGWIN__
/* kartu: commented to suppres compiler warning */
...
static unsigned int parse_directory(struct entry *root_entry, const char *name, struct entry **prev, loff_t *fslen_ub)
{
	...
	*endpath = '/';
	...
	for (dirindex = 0; dirindex < dircount; dirindex++) {
		...
		// kartu: added block that processes files ending with .lnk
		if (strcmp(path + strlen(path) - 4, ".lnk") == 0) {
			lnk = parseLnk(path);
			entry = xcalloc(1, sizeof(struct entry));

			entry = xcalloc(1, sizeof(struct entry));
			entry->name = xstrdup(dirent->d_name);
			entry->name[namelen - 4] = 0;
			entry->mode = S_IFLNK;
			entry->size = strlen(lnk->name);
			entry->uid = 0;
			entry->gid = 0;
			size = sizeof(struct cramfs_inode) + ((namelen + 3 - 4) & ~3);
			*fslen_ub += size;
			entry->uncompressed = lnk->name;
...
static struct entry *find_filesystem_entry(struct entry *dir, char *name, mode_t type)
{
    ...
    //printf("[dbg]  find_filesystem_entry: %s in %s\n",name,e->name);
	if (S_ISDIR(dir->mode)) {
		e = dir->child;
	}
	while (e) {
		/* Only bother to do the expensive strcmp on matching file types */
        //printf("[dbg]   checking: %s\n",e->name);
		if (1) {//type == (e->mode & S_IFMT) && e->name) {
			if (S_ISDIR(e->mode)) {
	...
	unsigned long mode, unsigned long rdev, struct entry *root, loff_t *fslen_ub)
{
	...
	if ((entry = find_filesystem_entry(parent, name, (mode & S_IFMT)))) {
        ...
        //printf("[dbg]  file not found, making new entry\n");	
...
static int interpret_table_entry(char *line, struct entry *root, loff_t *fslen_ub)
{
	...
	{
        //printf("[dbg] error parsing line: %s\n",line);
		return 1;
	}
	...
	case 'l':
		...
		break;
	default:
		error_msg_and_die("Unsupported file type for path %s",name);
	}
	free(name);
	return 0;
}
