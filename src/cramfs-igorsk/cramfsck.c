...
static int opt_devtbl = 0;
static FILE *fdevtbl = NULL;
...
static void usage(int status)
{
	...
	fprintf(stream, "usage: %s [-hv] [-x dir] [-d devtable] file\n"
		" -h         print this help\n"
		" -x dir     extract into dir\n"
    	" -x file    write devtable info to file\n"
		" -v         be more verbose\n"
		" file       file to test\n", progname);
...
static void print_devtbl(char type, struct cramfs_inode *i, char *name)
{
	//#<name>		<type>	<mode>	<uid>	<gid>	<major>	<minor>	<start>	<inc>	<count>
    char root[]="/";
    char* name1 = name+strlen(extract_dir);
    if (*name1==0) // root
        name1 = root;        
    fprintf(fdevtbl, "%s\t%c\t%04o\t%d\t%d\t", name1, type, i->mode, i->uid, i->gid);
    if (S_ISCHR(i->mode) || (S_ISBLK(i->mode))) {
		/* major/minor numbers can be as high as 2^12 or 4096 */
		fprintf(fdevtbl, "%4d\t%4d\t0\t0\t-\n", major(i->size), minor(i->size));
	}
	else {
		/* size be as high as 2^24 or 16777216 */
    	fprintf(fdevtbl, "-\t-\t-\t-\t-\n");
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "hx:vd:")) != EOF) {
		switch (c) {
		case 'h':
			usage(FSCK_OK);
		case 'x':
