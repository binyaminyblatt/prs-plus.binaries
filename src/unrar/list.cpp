          ...
          case FILE_HEAD:
            ...
            if (FileMatched)
            {
              if (Arc.IsArcDir()) {
                ...
                mprintf("f\t%d\t", Arc.NewLhd.FullUnpSize);
              }
              
              mprintf(Arc.NewLhd.FileName);
              mprintf("\n");
            }
            break;
#ifndef SFX_MODULE
          case SUB_HEAD:
            mprintf("Listing subheader ");
            ListOldSubHeader(Arc);
            break;
#endif
          case NEWSUB_HEAD:
            mprintf("Listing newsubheader ");
            ListNewSubHeader(Cmd,Arc,false);
            break;
        }
        Arc.SeekToNext();
      }
