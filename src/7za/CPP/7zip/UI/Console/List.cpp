...
public:
  ...
  HRESULT PrintItemInfo_prsp(const CArc &arc, UInt32 index);
...
HRESULT CFieldPrinter::PrintItemInfo_prsp(const CArc &arc, UInt32 index)
{
      
  // Type (f - file, d - directory)
  bool isFolder;
  RINOK(IsArchiveItemFolder(arc.Archive, index, isFolder));
  // kartu removed attribute list, setting f for file, d for directory
  //char s[8];
  //GetAttribString(attrib, isFolder, s);
  //g_StdOut << s;
  if (isFolder) {
  ...
  // Size
  NCOM::CPropVariant prop;
  RINOK(arc.Archive->GetProperty(index, kpidSize, &prop));
  UInt64 unpackSize;
  if (!GetUInt64Value(arc.Archive, index, kpidSize, unpackSize)) {
    unpackSize = 0;
  }
  g_StdOut << unpackSize << "\t";
    ...
    UInt64 &numErrors)
{
  ...
  for (int i = 0; i < numArcs; i++)
  {
    ...
    for (UInt32 i = 0; i < numItems; i++)
    {
      ...
      if (prspMode) {
        fieldPrinter.PrintItemInfo_prsp(arc, i);
      } else {
